...
class Booking(models.Model):
    ...
    check_in_status = models.CharField(
        max_length=20, choices=CHECK_IN_STATUS, default="not_checked_in"
    )
    ...
    hourly_start = models.DateTimeField(
        null=True, blank=True, help_text="Scheduled start time for hourly package"
    )
    hourly_end = models.DateTimeField(
        null=True, blank=True, help_text="Scheduled end time for hourly package"
    )
    extended_until = models.DateTimeField(
        null=True, blank=True, help_text="Extended end time if user requests extension"
    )
    late_fee_applied = models.BooleanField(
        default=False, help_text="Whether late fee was applied for overtime"
    )
