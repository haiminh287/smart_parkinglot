...
IMAGES_DIR = os.path.join(
    os.path.dirname(__file__), "ai-service-fastapi", "app", "images"
)
...
def test_register_vehicle(user_id):
    ...
    try:
        ...
        _conn = pymysql.connect(
            ...
            port=3307,
            user="parksmartuser",
            password="parksmartpass",
            database="parksmartdb",
        )
        _cur = _conn.cursor()
        _cur.execute(
            "DELETE FROM vehicle WHERE license_plate = %s AND user_id != %s",
            (TEST_PLATE, user_id),
        )
        if _cur.rowcount > 0:
            log(
                f"  Cleaned up stale vehicle for plate {TEST_PLATE} (prev test run)",
                "WARN",
            )
        _conn.commit()
        _conn.close()
    ...
    if r.status_code == 201:
        ...
        r2 = requests.get(
            f"{VEHICLE_URL}/vehicles/", headers=gw_headers(user_id, TEST_EMAIL)
        )
        ...
        if not vehicle_id and vehicles:
            ...
            unique_plate = (
                f"51E-{uuid.uuid4().hex[:3].upper()}.{uuid.uuid4().hex[:2].upper()}"
            )
            log(
                f"  Plate owned by another user. Registering new plate: {unique_plate}",
                "WARN",
            )
            r3 = requests.post(
                f"{VEHICLE_URL}/vehicles/",
            ...
            if r3.status_code == 201:
                ...
                log(
                    f"  Registered alternative vehicle: {unique_plate} (id={vehicle_id})"
                )
...
def test_find_parking(user_id):
    ...
    # List all lots
    r = requests.get(
        f"{PARKING_URL}/parking/lots/", headers=gw_headers(user_id, TEST_EMAIL)
    )
    ...
    for lot in lots_list:
        ...
        log(
            f"     Slots:     {lot.get('availableSlots', '?')}/{lot.get('totalSlots', '?')}"
        )
    ...
    for z in car_zones[:3]:
        log(
            f"  🅿️  Zone {z.get('name')}: {z.get('availableSlots')}/{z.get('capacity')} available"
        )
...
def test_package_pricing(user_id):
    ...
    r = requests.get(
        f"{BOOKING_URL}/bookings/packagepricings/",
        headers=gw_headers(user_id, TEST_EMAIL),
    )
...
def test_create_booking_online(user_id, vehicle_id, lot_id, zone_id, floor_id, slot_id):
    ...
    check(
        ...
        "Payment status is pending/processing",
        f"got={payment_status}",
    )
    check(
        checkin_status == "not_checked_in",
        "Check-in status is not_checked_in",
...
def test_pricing_packages(user_id, vehicle_id, lot_id, zone_id):
    ...
    for pkg_type, duration in packages:
        ...
        if r.status_code == 201:
            ...
            requests.post(
                f"{BOOKING_URL}/bookings/{bid}/cancel/",
                headers=gw_headers(user_id, TEST_EMAIL),
            )
        else:
...
def test_payment_online(user_id, booking_id):
    ...
    r = requests.post(
        f"{PAYMENT_URL}/api/payments/initiate/",
        json={
    ...
    if r.status_code in [200, 201]:
        ...
        r2 = requests.post(
            f"{PAYMENT_URL}/api/payments/verify/{payment_id}/",
            json={
                "transactionId": tx_id,
                "gatewayResponse": {
                    "status": "success",
...
def test_ai_scan_plate(image_file, expected_plate=None):
    ...
    with open(img_path, "rb") as f:
        r = requests.post(
            f"{AI_URL}/ai/parking/scan-plate/",
    ...
    if r.status_code == 200:
        ...
        if expected_plate:
            ...
            check(
                match,
                f"Plate matches expected '{expected_plate}'",
                f"got='{plate_text}'",
            )
...
def test_checkin_matching(user_id, booking_id, qr_data):
    ...
    check(
        plate_match,
        "Plate matched booking",
        f"ocr={ocr_plate} vs booking={booking_plate}",
    )

    # Verify booking status updated
    r2 = requests.get(
        f"{BOOKING_URL}/bookings/{booking_id}/", headers=gw_headers(user_id, TEST_EMAIL)
    )
    if r2.status_code == 200:
        bdata = r2.json()
...
def test_checkin_wrong_plate(user_id, booking_id, qr_data):
    ...
    if isinstance(detail, dict):
        ...
        check(
            ...
            "Plate mismatch error returned",
            f"error={error_type}",
        )
        check(
            r.status_code == 400,
            "HTTP 400 for plate mismatch",
            f"status={r.status_code}",
        )
        ...
        check(
            not success or not plate_match,
...
def test_checkout_via_ai(user_id, booking_id, qr_data):
    ...
    booking_data = (
        raw_booking.get("booking", raw_booking)
        if isinstance(raw_booking, dict)
        else raw_booking
    )
    check_in_status = booking_data.get(
        "checkInStatus", booking_data.get("check_in_status", "")
    )
...
def test_checkout_direct(user_id, booking_id):
    ...
    r = requests.post(
        f"{BOOKING_URL}/bookings/{booking_id}/checkout/",
        headers=gw_headers(user_id, TEST_EMAIL),
    )
...
def main():
    ...
    log(
        f"Available images: {os.listdir(IMAGES_DIR) if os.path.isdir(IMAGES_DIR) else 'NOT FOUND'}"
    )
    ...
    try:
        ...
        if avail_slots:
            ...
            if r_b3.status_code == 201:
                b3_raw = r_b3.json()
                b3_data = (
                    b3_raw.get("booking", b3_raw)
                    if isinstance(b3_raw, dict)
                    else b3_raw
                )
                ...
                log(
                    f"  Could not create booking for wrong-plate test: {r_b3.status_code} {r_b3.text[:200]}",
                    "WARN",
                )
        else:
    ...
    log(
        f"Result:  {'ALL PASSED ✅' if failed == 0 else f'{failed} FAILURES ❌'}",
