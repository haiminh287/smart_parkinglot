...
DATABASES = {
    "default": {
        ...
        "PASSWORD": os.environ[
            "DB_PASSWORD"
        ],  # Required: set DB_PASSWORD in environment
...
if ENV.lower() == "production":
    if not SESSION_COOKIE_DOMAIN:
        raise ImproperlyConfigured(
            "SESSION_COOKIE_DOMAIN is required when ENV=production"
        )
    if not SESSION_COOKIE_SECURE:
        raise ImproperlyConfigured(
            "SESSION_COOKIE_SECURE must be true when ENV=production"
        )
    if not CSRF_COOKIE_SECURE:
        raise ImproperlyConfigured(
            "CSRF_COOKIE_SECURE must be true when ENV=production"
        )
    if not CORS_ALLOWED_ORIGINS:
        raise ImproperlyConfigured(
            "CORS_ALLOWED_ORIGINS is required when ENV=production"
        )
    for origin in CORS_ALLOWED_ORIGINS:
        lower_origin = origin.lower()
        if not lower_origin.startswith("https://"):
            raise ImproperlyConfigured(
                "CORS_ALLOWED_ORIGINS must use https:// when ENV=production"
            )
        if "localhost" in lower_origin or "127.0.0.1" in lower_origin:
            raise ImproperlyConfigured(
                "CORS_ALLOWED_ORIGINS cannot include localhost when ENV=production"
            )
