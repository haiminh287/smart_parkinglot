...
from infrastructure.models import CarSlot, ProcessedEvent
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("Listening for slot.status_changed events...")
        )
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    if slot_id and status:
                        updated = CarSlot.objects.filter(id=slot_id).update(
                            status=status
                        )
                        if updated:
