    ...
    def get_queryset(self):
                ...
                for lot in queryset:
                    if lot.latitude and lot.longitude:
                        ...
                        lon1, lat1, lon2, lat2 = map(
                            radians, [lng, lat, lot.longitude, lot.latitude]
                        )
                        dlon = lon2 - lon1
                        dlat = lat2 - lat1
                        a = (
        ...
        if vehicle_type:
            ...
            queryset = queryset.filter(
                floors__zones__vehicle_type=vehicle_type
            ).distinct()
        ...
        return Response(
            {
                ...
                "occupancy_rate": round(
                    (occupied / total * 100) if total > 0 else 0, 1
                ),
                "by_vehicle_type": {
                    "car": {
                        "total": car_total,
                        ...
                        "occupancy_rate": round(
                            (
                                (car_total - car_available) / car_total * 100
                                if car_total > 0
                                else 0
                            ),
                            1,
                    ...
                    "motorbike": {
                        ...
                        "occupancy_rate": round(
                            (
                                (bike_total - bike_available) / bike_total * 100
                                if bike_total > 0
                                else 0
                            ),
                            1,
...
class CarSlotViewSet(viewsets.ModelViewSet):
    ...
    def check_slots_availability(self, request):
        ...
        try:
            ...
            BOOKING_SERVICE = os.environ.get(
            ...
            for slot in slots:
                is_available = (
                    slot.status == "available" and str(slot.id) not in booked_slot_ids
                )
    ...
    def update_status(self, request, pk=None):
        ...
        lot.available_slots = CarSlot.objects.filter(
            zone__floor__parking_lot=lot, status="available"
        ).count()
        ...
        return Response(
            {
                ...
                or f"rtsp://camera-server/{camera.id}/stream",
