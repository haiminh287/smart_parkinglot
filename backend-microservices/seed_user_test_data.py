...
ZONES = {
    ...
    "C": {
        "id": "86bf498a7b084f4cbc2d3b0efca0c742",
        "name": "Zone C",
        "type": "Motorbike",
        "lot": "parksmart",
        "floor_id": "30098e7d888b4743b0068a331d04b26d",
        "floor_level": 2,
    },
    "D": {
        "id": "5ef51d4b5cb24dc987eecb1840bc4258",
        "name": "Zone D",
        "type": "Motorbike",
        "lot": "parksmart",
        "floor_id": "30098e7d888b4743b0068a331d04b26d",
    ...
    "E": {
        "id": "9a7588eff5b64b5ab5ba10e55d20223c",
        "name": "Zone E",
        "type": "Car",
        "lot": "parksmart",
        "floor_id": "610fbad6a32445e3ac04917b361f064d",
        "floor_level": 3,
    },
    # Vincom B1
    "V1": {
...
def get_available_slot(
    conn: pymysql.Connection, zone_id: str, exclude_ids: list[str]
) -> tuple[str, str] | None:
    with conn.cursor() as cur:
...
def seed_bookings(
    ...
    bookings = [
        ...
        {
            "lot_id": lot_ps["id"],
            "lot_name": lot_ps["name"],
            "floor_id": ZONES["E"]["floor_id"],
            "floor_level": ZONES["E"]["floor_level"],
            "zone": ZONES["E"],
            "slot": s_e or (make_uuid(), "E-01"),
            "vehicle_id": car_id,
            "plate": car_plate,
            "vtype": "Car",
            ...
            "price": 30000,
            "desc": "Completed/cash/checked_out - ParkSmart Tang 3",
        },
        # 2) Completed + momo — 5 days ago, daily package
        {
            "lot_id": lot_vc["id"],
            "lot_name": lot_vc["name"],
            "floor_id": lot_vc["floor_id"],
            "floor_level": lot_vc["floor_level"],
            "zone": ZONES["V1"],
            "slot": s_v1 or (make_uuid(), "V1-01"),
            "vehicle_id": car_id,
            "plate": car_plate,
            "vtype": "Car",
            ...
            "price": 120000,
            "desc": "Completed/momo/daily - Vincom Tang B1",
        },
        # 3) Active — checked_in (motorbike)
        {
            "lot_id": lot_ps["id"],
            "lot_name": lot_ps["name"],
            "floor_id": ZONES["C"]["floor_id"],
            "floor_level": ZONES["C"]["floor_level"],
            "zone": ZONES["C"],
            "slot": s_c or (make_uuid(), "C-01"),
            "vehicle_id": moto_id,
            "plate": moto_plate,
            "vtype": "Motorbike",
            ...
            "checked_out": None,
            "price": 10000,
            "desc": "Active/on_exit/checked_in - ParkSmart Tang 2",
        },
        # 4) Upcoming — booked for day after tomorrow
        {
            ...
            "checked_out": None,
            "price": 20000,
            "desc": "Upcoming/online/not_checked_in - Vincom Tang B1",
        },
        # 5) Cancelled motorbike — 2 days ago
        {
            "lot_id": lot_ps["id"],
            "lot_name": lot_ps["name"],
            "floor_id": ZONES["D"]["floor_id"],
            "floor_level": ZONES["D"]["floor_level"],
            "zone": ZONES["D"],
            "slot": s_d or (make_uuid(), "D-01"),
            "vehicle_id": moto_id,
            "plate": moto_plate,
            "vtype": "Motorbike",
            ...
            "checked_out": None,
            "price": 10000,
            "desc": "Cancelled/refunded - ParkSmart Tang 2",
        },
    ]
    ...
    if active_slot_id:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE car_slot SET status='occupied' WHERE id=%s", (active_slot_id,)
            )
        conn.commit()
...
def seed_payments(
    ...
    with conn.cursor() as cur:
        for p in payment_data:
            ...
            completed_at = (
                fmt_micro(NOW - timedelta(days=10 - idx * 3))
            ...
            print(
                f"  OK - {p['method']} {p['amount']}d tx={tx_id} status={p['status']}"
            )
    conn.commit()
...
def seed_notifications(conn: pymysql.Connection, user_id: str) -> None:
    ...
    notifs = [
        (
            "booking",
            "Dat cho thanh cong",
            "Slot E-01 tai ParkSmart Tower da duoc xac nhan. Giu cho den 10 ngay truoc.",
            True,
        ...
        (
            ...
            "Thanh toan 120.000d goi ngay tai Vincom Center qua Momo.",
            True,
        ...
        (
            "booking",
            "Booking sap toi",
            "Nhac nho: ban co booking tai Vincom Center sau 2 ngay luc 08:00.",
            False,
        ...
        (
            "system",
            "Khuyen mai thang 3",
            "Giam 20% cho moi booking tu 25-31/03. Ap dung cho goi theo ngay!",
            False,
...
def print_summary(conn: pymysql.Connection, user_id: str) -> None:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT email, first_name, last_name, phone, role FROM users_user WHERE id=%s",
            (user_id,),
        )
        r = cur.fetchone()
        if r:
        ...
        for r in rows:
            print(
                f"    [{r[2]:18s}] slot={r[0]:6s} lot={r[1]} pay={r[3]} price={r[4]} date={r[5]}"
            )

        cur.execute(
            "SELECT COUNT(*) FROM payments_payment WHERE user_id=%s", (user_id,)
        )
        print(f"  Payments: {cur.fetchone()[0]} records")

        cur.execute(
            "SELECT COUNT(*) FROM notifications_notification WHERE user_id=%s",
            (user_id,),
        )
