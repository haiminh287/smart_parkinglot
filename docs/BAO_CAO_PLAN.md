...
**Frontend.** Trong môi trường phát triển cục bộ, giao diện được khởi chạy bằng `npm install` và `npm run dev`, Vite dev server tự động phục vụ tại cổng 8080 và proxy các request đến gateway, realtime service và AI service. Trong môi trường triển khai production, giao diện được cài đặt qua `npm ci` và build thành bundle tĩnh bằng `npm run build`; ba biến môi trường bắt buộc (`VITE_API_URL`, `VITE_WS_URL`, `VITE_GATEWAY_SECRET`) phải có giá trị hợp lệ trước khi build, nếu không quá trình build sẽ thất bại. Bundle `dist/` được phục vụ bởi Nginx ở cổng 80.
...
**Mô phỏng 3D Digital Twin.** Bản sao kỹ thuật số được xây dựng trên Unity 2022.3 LTS, mở dự án bằng Unity Hub và sử dụng kịch bản (scene) chính cùng các script C# có sẵn trong thư mục `Assets/Scripts`. Trước khi nhấn Play, tác giả seed dữ liệu tương thích vào cơ sở dữ liệu bằng các script `seed_unity_test_data.py` và `seed_unity_slots.py` để bảo đảm mã ô đỗ trong scene khớp với bãi xe đang được Unity truy vấn từ backend. Khi chạy, môi trường mô phỏng tạo ra 158 ô đỗ và sáu camera ảo, gửi khung hình JPEG đến AI Service ở cổng 8009 và kết nối WebSocket tới Realtime Service để nhận cập nhật trạng thái slot — thay thế hoàn toàn cho bãi xe thật trong giai đoạn nghiên cứu.
...
Hệ thống được kiểm thử qua mười một kịch bản nghiệp vụ phản ánh đúng vòng đời sử dụng của một bãi giữ xe (Bảng 4.2). Tất cả kịch bản đều thực hiện thành công ở môi trường vận hành công khai.
...
| 10  | Mô phỏng đầu cuối trên Unity Digital Twin         | ✅ Sáu camera ảo gửi frame đến AI, 158 ô đỗ đồng bộ realtime với backend           |
| 11  | Truy cập từ Internet công cộng                    | ✅ Phản hồi HTTP/2 200 với chứng chỉ TLS                                           |
...
### 5.1.2. Vấn đề đã được giải quyết
...
### 5.2.2. Giải pháp và định hướng phát triển
...
[1] A. Vaswani, N. Shazeer, N. Parmar, J. Uszkoreit, L. Jones, A. N. Gomez, Ł. Kaiser, and I. Polosukhin, "Attention is all you need," in _Proc. 31st Conf. Neural Inf. Process. Syst. (NIPS)_, Long Beach, CA, USA, Dec. 2017, pp. 5998–6008.
Available: https://papers.nips.cc/paper/2017/hash/3f5ee243547dee91fbd053c1c4a845aa-Abstract.html

[2] J. Redmon, S. Divvala, R. Girshick, and A. Farhadi, "You only look once: Unified, real-time object detection," in _Proc. IEEE Conf. Comput. Vis. Pattern Recognit. (CVPR)_, Las Vegas, NV, USA, Jun. 2016, pp. 779–788.
doi: 10.1109/CVPR.2016.91
Available: https://doi.org/10.1109/CVPR.2016.91

[3] M. Tan and Q. V. Le, "EfficientNetV2: Smaller models and faster training," in _Proc. 38th Int. Conf. Mach. Learn. (ICML)_, vol. 139, Jul. 2021, pp. 10096–10106.
Available: https://proceedings.mlr.press/v139/tan21a.html

[4] M. Li, T. Lv, J. Chen, L. Cui, Y. Lu, D. Florencio, C. Zhang, Z. Li, and F. Wei, "TrOCR: Transformer-based optical character recognition with pre-trained models," in _Proc. AAAI Conf. Artif. Intell._, vol. 37, no. 11, Washington, DC, USA, Feb. 2023, pp. 13094–13102.
doi: 10.1609/aaai.v37i11.26538
Available: https://doi.org/10.1609/aaai.v37i11.26538

[5] N. Reimers and I. Gurevych, "Sentence-BERT: Sentence embeddings using Siamese BERT-networks," in _Proc. Conf. Empirical Methods Natural Lang. Process. and 9th Int. Joint Conf. Natural Lang. Process. (EMNLP-IJCNLP)_, Hong Kong, China, Nov. 2019, pp. 3982–3992.
doi: 10.18653/v1/D19-1410
Available: https://aclanthology.org/D19-1410

[6] P. Lewis, E. Perez, A. Piktus, F. Petroni, V. Karpukhin, N. Goyal, H. Küttler, M. Lewis, W.-t. Yih, T. Rocktäschel, S. Riedel, and D. Kiela, "Retrieval-augmented generation for knowledge-intensive NLP tasks," in _Proc. 34th Conf. Neural Inf. Process. Syst. (NeurIPS)_, Vancouver, BC, Canada, Dec. 2020, pp. 9459–9474.
